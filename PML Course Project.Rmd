...
attach("./objects/modFit")
```

And finally we create our prediction model by running a randomforests prediction algorithm on our clean and processed training set. 
...
modFit$finalModel
```
As we can see, our expected out of sample error rate, using random forests, is approximately 0.76%. We can calculate this ourselves, by dividing the sum of our incorrect predictions `sum(modFit$finalModel$confusion)-sum(diag(modFit$finalModel$confusion))` by the sum of our correct predictions `sum(diag(modFit$finalModel$confusion))`:

```{r, echo=FALSE}
(sum(modFit$finalModel$confusion)-sum(diag(modFit$finalModel$confusion)))/sum(diag(modFit$finalModel$confusion))
```
...
```
The error rate we obtain in our test sample is 0.82%. Not bad. 
